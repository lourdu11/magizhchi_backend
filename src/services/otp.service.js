...
const sendOTP = async (identifier, purpose = 'register') => {
  ...
  const isPhoneId = /^\d{10,}$/.test(identifier.replace(/\D/g, ''));

  console.log('DEBUG: Flow Detection -> isEmail:', isEmailId, 'isPhone:', isPhoneId);
...
// ─── Verify OTP ───────────────────────────────────────────────
const verifyOTP = async (identifier, otp, purpose = 'register', deleteAfter = true) => {
